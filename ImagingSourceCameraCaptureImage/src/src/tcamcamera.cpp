...
TcamCamera::create_pipeline()
{
    ...
    g_object_set(capturesink_, "sync", true, "max-buffers", 4, "drop", true, nullptr);
...
void TcamCamera::set_trigger()
{
    GValue trigger_val = {};
    g_value_init(&trigger_val, G_TYPE_BOOLEAN);
    g_value_set_boolean(&trigger_val, FALSE);

    tcam_prop_set_tcam_property(TCAM_PROP(tcambin_), "Trigger Mode", &trigger_val);    
}

void TcamCamera::trigger()
{
    GValue trigger_val = {};
    g_value_init(&trigger_val, G_TYPE_BOOLEAN);
    g_value_set_boolean(&trigger_val, TRUE);
    gboolean r = tcam_prop_set_tcam_property(TCAM_PROP(tcambin_), "Software Trigger", &trigger_val);
  
}


void TcamCamera::set_auto_exposure()
{
